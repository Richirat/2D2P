...
def _is_legacy_rotary_log(path_txt):
    """True if first non-empty data line uses legacy 'Rot=' format."""
    with open(path_txt, "r", encoding="utf-8") as f:
        for line in f:
            s = line.strip()
            if not s:
...
def read_rotary_log_legacy(path_txt, tA_wall=None, window_sec=1.0):
    """Read legacy rotary log: '<date> <time> Rot=<deg> Trigger=<v>'.
    Returns (dt_list, angle_list)."""
    dt_list, angle_list = [], []
    t_min = t_max = None
    if tA_wall is not None:
        a = np.asarray(tA_wall)
        if a.size > 0:
            if np.issubdtype(a.dtype, np.datetime64):
                def _to_dt(x):
                    ns = x.astype("datetime64[ns]").astype("int64")
                    return dt.datetime(1970, 1, 1) + dt.timedelta(microseconds=ns / 1000.0)
                t_min = _to_dt(a.min()); t_max = _to_dt(a.max())
            else:
                t_min = min(tA_wall); t_max = max(tA_wall)
            w = float(window_sec or 0.0)
            t_min -= dt.timedelta(seconds=w); t_max += dt.timedelta(seconds=w)
    with open(path_txt, "r", encoding="utf-8") as f:
        for line in f:
            line = line.strip()
            if not line:
            ...
            if len(parts) < 3:
                continue
            try:
                t = dt.datetime.strptime(parts[0] + " " + parts[1], "%Y-%m-%d %H:%M:%S.%f")
            except ValueError:
                try:
                    t = dt.datetime.strptime(parts[0] + " " + parts[1], "%Y-%m-%d %H:%M:%S")
                except ValueError:
                    continue
            if t_min is not None and (t < t_min or t > t_max):
                continue
            ...
            for tok in parts[2:]:
                if tok.startswith("Rot="):
                    ...
                    try:
                        ...
                        ang = float("nan")
                    break
            dt_list.append(t); angle_list.append(ang)
    return dt_list, angle_list
...
def get_frame_angles_from_rotary(tif_path, rotary_log_path, window_sec=1.0):
    ...
    if _is_legacy_rotary_log(rotary_log_path):
        dt_list, angle_list = read_rotary_log_legacy(rotary_log_path, tA_wall=tA_wall, window_sec=window_sec)
    else:
